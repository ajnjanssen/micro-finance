...
export default function BudgetSettingsSection() {
  ...
  const loadSettings = async () => {
    try {
      ...
      if (config.incomeSources) {
        config.incomeSources.forEach((income: any) => {
          if (income.isActive) {
  ...
  const handleSave = async () => {
    ...
    if (Math.abs(total - 100) > 0.01) {
      alert(
        `Percentages moeten optellen tot 100%. Huidige totaal: ${total.toFixed(
          1
        )}%`
      );
      return;
        ...
        <div className="card-body">
          ...
          >
            <div className="flex items-center justify-between w-full">
              <span className="font-semibold">Totaal: {total.toFixed(1)}%</span>
              {!isValid && <span className="text-sm">⚠️ Moet 100% zijn</span>}
              {isValid && <span className="text-sm">✅ Perfect!</span>}
            </div>
          </div>
      ...
      <div className="card bg-base-200">
        <div className="card-body">
          <h3 className="font-bold mb-4">
            Voorbeeld bij €{income.toFixed(2)} inkomen
          </h3>
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span>🏠 Needs</span>
              <span className="font-semibold">
                €{((income * percentages.needs) / 100).toFixed(2)}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span>🎉 Wants</span>
              <span className="font-semibold">
                €{((income * percentages.wants) / 100).toFixed(2)}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span>💰 Savings</span>
              <span className="font-semibold">
                €{((income * percentages.savings) / 100).toFixed(2)}
              </span>
            </div>
          </div>
        </div>
