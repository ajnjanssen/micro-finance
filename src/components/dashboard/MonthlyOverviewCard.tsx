...
export function MonthlyOverviewCard({ overview }: MonthlyOverviewCardProps) {
	...
	const hasActualData =
		overview.actualIncome !== undefined || overview.actualExpenses !== undefined;
		...
		<div className="card bg-base-100 shadow">
			<div className="card-body p-0">
				<div className="flex items-center justify-between mb-4">
					...
					<div className="tooltip tooltip-left" data-tip="Dit toont je geplande inkomsten en uitgaven voor deze maand. 'Geconfigureerd' = terugkerende transacties in je budget. 'Werkelijk' = voltooide transacties.">
				...
				<div className="stats shadow w-full">
					<div className="stat border-l-primary border-10 !border-r-none">
					...
					<div className="stat">
						...
						<div
							className={`stat-value ${overview.netAmount >= 0 ? "text-success" : "text-error"}`}
						>
						...
						{hasActualData &&
							...
							overview.actualExpenses !== undefined && (
								<div className="stat-desc text-xs mt-1">
									Werkelijk:{" "}
									{formatCurrency(overview.actualIncome - overview.actualExpenses)}
								</div>
							)}
					</div>
				</div>
				<div className="mt-4 p-3 bg-info/10 rounded-lg border-l-4 border-info">
					<p className="text-sm text-base-content/70">
						<strong>💡 Uitleg:</strong> Deze bedragen zijn gebaseerd op je terugkerende inkomsten en uitgaven. 
						Het "Netto Groei" bedrag ({formatCurrency(overview.netAmount)}) is hoeveel je vermogen elke maand groeit 
